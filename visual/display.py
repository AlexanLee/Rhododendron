    ...
    def render(self):
        ...
        if self.conf.contains('ratings'):
            ...
            html += "<b>User Count</b>: {user}<br><b>Item Count</b>: {item}<br><b>Record Count</b>: {record}<br><b>Global Mean</b>: {mean}</div>\n" \
                .format(user=str(len(self.dao.user)), item=str(len(self.dao.item)),
                        record=str(len(self.dao.trainingData)),
                        mean=str(round(denormalize(self.dao.globalMean, self.dao.rScale[-1],
                                                   self.dao.rScale[0]), 3)))
